...
MyPnky.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    ...
    if (message.message.startsWith('pinky') && !plkser.message.includes('{pinky}')) {        
        ...
        await axios.get('http://api.brainshop.ai/get?bid=161399&key=gK0fwEmt07zvnPXH&uid=' + unique_ident + '&msg=' + uren).then(async (response) => {
            var fins = ''                           
            if (conf.LANG !== 'EN') {
    ...
    try {
        ...
        convertToWav(file)
            .on('end', async () => {
                ...
                await axios.get('http://api.brainshop.ai/get?bid=161399&key=gK0fwEmt07zvnPXH&uid=' + unique_ident + '&msg=' + son).then(async (response) => {
                    var trmsg = ''
